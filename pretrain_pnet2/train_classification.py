...
from pretrain_pnet2.data_utils.ModelNetDataLoader import ModelNetDataLoader
...
def test(model, loader, num_class=40):
    ...
    for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
        ...
        pred = classifier.forward_clf(points)
...
def main(args):
    ...
    data_path = '/data/meta-ScanNet/modelnet40_normal_resampled/'
    ...
    classifier = single_object_encoder(768)  # single_object_encoder takes [bs, num_points, 6]
    criterion = get_loss()
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader),
            ...
            points = torch.Tensor(points)  # this is [bs, num_points, 6]
            # points = points.transpose(2, 1)

            if not args.use_cpu:
                points, target = points.cuda(), target.cuda()

            pred = classifier.forward_clf(points)  # points
