...
from in_out.scannet_scan import ScannetScan, ScannetDataset
...
运行: 
python prepare_scannet_data.py -top-scan-dir /data/ScanNet/scans -top-save-dir /data/meta-ScanNet/pkl_nr3d

python prepare_scannet_data.py -top-scan-dir /data/ScanNet/scans -top-save-dir /data/meta-ScanNet/pkl_sr3d --process-only-zero-view False

Use the default options for nr3d. you can safely ignore the scene0009_00 warning.
"""
...
if __name__ == '__main__':
    ...
    idx_to_semantic_class_file = '../data/mappings/scannet_idx_to_semantic_class.json'  # 0: background, 1: wall ...
    instance_class_to_semantic_class_file = '../data/mappings/scannet_instance_class_to_semantic_class.json'  # armchair: chair, 将小类物品归为大类
    axis_alignment_info_file = '../data/scannet/scans_axis_alignment_matrices.json'  # 对齐不同场景，每个场景给了4x4 matrix
    ...
    for i, data in enumerate(pool.imap(scannet_loader, all_scan_ids, chunksize=chunks)):
        all_scans[all_scan_ids[i]] = data  # 并行载入所有scan，data是ScannetScan对象
    ...
    pickle_data(save_file, scannet, all_scans)  # cPickle保存scannet和all_scans两个对象
