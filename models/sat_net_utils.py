...
def single_epoch_train(model, data_loader, criteria, optimizer, device, pad_idx, args, epoch=None):
    ...
    for i, batch in pbar:
        ...
        if args.lang_cls_alpha > 0:
            ...
            lang_loss_mtr.update(all_losses['lang_clf_loss'].item(), batch_size)
    ...
    metrics['train_lang_clf_loss'] = lang_loss_mtr.avg
...
def compute_losses(batch, res, criterion_dict, args):
    ...
    # lang_clf_loss = lang_logits <-> target_obj_classification
...
def dump_batch(data_loader, args, epoch=None, prev_iters=10, dst_path='dump_batch.pt'):
    """
    dump出指定的数据结构用于BERT Debug
    """
    np.random.seed(
        args.random_seed + epoch)  # call this to change the sampling of the point-clouds, in a time-invariant way
    batch_keys = make_batch_keys(args)
