...
from in_out.vocabulary import build_vocab, Vocabulary
...
def load_referential_data(args, referit_csv, scans_split, scanref=False):
    """
    :param args:
    :param referit_csv:
    :param scans_split:
    ...
    referit_data.reset_index(drop=True, inplace=True)
    print(
        'Dropping utterances with more than {} tokens, {}->{}'.format(args.max_seq_len, n_original, len(referit_data)))

    if scanref:
        referit_data = load_scanref_data(args, referit_data, '../../text_data/scanref_raw/ScanRefer_filtered.json',
                                         '../../text_data/scanref_raw/ScanRefer_filtered_train.txt')
    ...
    if args.augment_with_sr3d is not None:
        ...
        # referit_data = pd.concat([referit_data, referit_data, referit_data, sr3d], axis=0)    ## balance sample?; also duplicate val set, sub should be same?
...
def load_scanref_data(args, referit_data, scanref_json, scanref_trainidx):
    import json
    ## load index from referit3D (nr3d)
    index_dict = {}
    for row in range(referit_data.shape[0]):
        data = referit_data.iloc[row]
        scan_id, target_id = data['scan_id'], data['target_id']
        key = '%s_%d' % (scan_id, target_id)
        if key not in index_dict:
            index_dict[key] = row
    # print(len(index_dict))  ## unique targets 5867

    ## load scanref info
    scanref_json = json.load(open(scanref_json, 'r'))
    scanref_trainidx = [x.strip() for x in list(open(scanref_trainidx, 'r'))]
    scanref_data = pd.DataFrame(columns=referit_data.columns, index=range(0, len(scanref_json)))
    for ii in range(len(scanref_json)):
        jsondata = scanref_json[ii]
        key = '%s_%d' % (jsondata['scene_id'], int(jsondata['object_id']))
        ## 19593 in 26580 out
        if key in index_dict:
            row = referit_data.iloc[index_dict[key]]
            scanref_data.iloc[ii] = row
            scanref_data.iloc[ii]['tokens'] = jsondata['token']
            scanref_data.iloc[ii]['utterance'] = jsondata['description']
            scanref_data.iloc[ii]['is_train'] = (jsondata['scene_id'] in scanref_trainidx)
        else:
            row = referit_data.iloc[0]  ## a random template
            scanref_data.iloc[ii] = row
            scanref_data.iloc[ii]['tokens'] = jsondata['token']
            scanref_data.iloc[ii]['utterance'] = jsondata['description']
            scanref_data.iloc[ii]['is_train'] = (jsondata['scene_id'] in scanref_trainidx)
            scanref_data.iloc[ii]['scan_id'] = jsondata['scene_id']
            scanref_data.iloc[ii]['target_id'] = int(jsondata['object_id'])
            scanref_data.iloc[ii]['instance_type'] = jsondata['object_name']
            ## seems that have to load scans to get distractor id, so given up for now; only influence scanref analyses numbers (rand guess acc)
            ## TODO: solve by replace this function with a separately generated scanref.csv in the nr3d.csv format
            scanref_data.iloc[ii]['stimulus_id'] = '%s-%s-2-%d-0' % (
            jsondata['scene_id'], jsondata['object_name'], int(jsondata['object_id']))
    scanref_data['is_train'] = scanref_data['is_train'].astype(bool)
    scanref_data['target_id'] = scanref_data['target_id'].astype('int64')
    return scanref_data
