...
FILETYPES = ['.aggregation.json', '.sens', '.txt', '_vh_clean.ply', '_vh_clean_2.0.010000.segs.json', '_vh_clean_2.ply',
             '_vh_clean.segs.json', '_vh_clean.aggregation.json', '_vh_clean_2.labels.ply',
# remove 2D instance label
             '_2d-instance.zip',
...
DATA_EFFICIENT_FILES = ['limited-reconstruction-scenes.zip', 'limited-annotation-points.zip', 'limited-bboxes.zip',
                        '1.7MB']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tf_semantic', action='store_true',
    ...
    if args.task_data:  # download task data
        ...
        print('You are downloading the data efficient task files' + ' which requires ' + DATA_EFFICIENT_FILES[
            -1] + ' of space.')
        for k in range(len(DATA_EFFICIENT_FILES) - 1):
            download_file(os.path.join(BASE_URL, RELEASE_TASKS, DATA_EFFICIENT_FILES[k]),
    ...
    with open('scannet_filelist.txt', 'w') as f:
        f.write('\n'.join(["{} {}".format(o[0], o[1]) for o in filelist]))
